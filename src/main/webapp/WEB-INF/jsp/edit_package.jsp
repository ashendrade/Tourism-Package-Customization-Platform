...
<head>
    ...
    <title>Edit Customized Itinerary - OCEANWAVE</title>
...
<div class="container">
    <div class="customize-layout">
        ...
        <div class="customize-form-card">
            <h1 class="mb-4">Modify Itinerary <code>#${pkg.id}</code></h1>
            
            <form id="customizeForm" action="${pageContext.request.contextPath}/packages/update/${pkg.id}" method="POST" onsubmit="prepareData()">
                ...
                <input type="hidden" name="hotelType" id="hiddenHotelType" value="${pkg.hotelType}">
                <input type="hidden" name="activities" id="hiddenActivities" value="${pkg.activities}">
                
                <div class="grid-2">
                    <div class="form-group">
                        <label>Accommodation Tier</label>
                        <select name="hotelTier" id="hotelTier" class="form-control" onchange="updatePrice()">
                            <option value="Budget" ${pkg.hotelTier == 'Budget' ? 'selected' : ''}>Budget (Guest Houses)</option>
                            <option value="Standard" ${pkg.hotelTier == 'Standard' ? 'selected' : ''}>Standard (3-4 Star Hotels)</option>
                            <option value="Luxury" ${pkg.hotelTier == 'Luxury' ? 'selected' : ''}>Luxury (5-Star Boutique)</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Duration (Days)</label>
                        <input type="number" name="duration" id="duration" class="form-control" value="${pkg.duration}" min="1" onchange="updatePrice()">
                    </div>
                </div>

                <h3 class="section-title"><i class="fas fa-tasks"></i> Selected Activities</h3>
                <div class="checkbox-grid">
                    <div class="checkbox-item">
                        <input type="checkbox" class="activity" value="3000" data-name="City Walking Tour" onchange="updatePrice()" ${not empty pkg.activities && pkg.activities.contains('City Walking Tour') ? 'checked' : ''}>
                        <span>City Walking Tour (+3k)</span>
                    </div>
                    <div class="checkbox-item">
                        <input type="checkbox" class="activity" value="15000" data-name="Adventure Trekking" onchange="updatePrice()" ${not empty pkg.activities && pkg.activities.contains('Adventure Trekking') ? 'checked' : ''}>
                        <span>Adventure Trekking (+15k)</span>
                    </div>
                    <div class="checkbox-item">
                        <input type="checkbox" class="activity" value="8000" data-name="Cultural Show" onchange="updatePrice()" ${not empty pkg.activities && pkg.activities.contains('Cultural Show') ? 'checked' : ''}>
                        <span>Cultural Show (+8k)</span>
                    </div>
                    <div class="checkbox-item">
                        <input type="checkbox" class="activity" value="12000" data-name="Safari Expedition" onchange="updatePrice()" ${not empty pkg.activities && pkg.activities.contains('Safari Expedition') ? 'checked' : ''}>
                ...
                <div class="form-group mt-4">
                    ...
                    <textarea name="specialRequest" class="form-control" rows="3" placeholder="Dietary needs, birthday surprises, extra beds...">${pkg.specialRequest}</textarea>
                </div>
                ...
                <div class="btn-actions">
                    ...
                    <a href="${pageContext.request.contextPath}/packages" class="btn-cancel"><i class="fas fa-arrow-left me-2"></i>Back</a>
                </div>
            </form>
        </div>
    </div>
